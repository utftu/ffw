...
глобальную подписку(formik, final-form). Это простой подход, который, однако, увеличивает число обновлений компонентов и 
это становится узким местом приложения с большим количеством полей на странице. 
...
2. Она должна иметь удобное api, схожее с популярными библиотеками(formik, final-form) 
3. Должна иметь с популярными валидаторами

И в formik-е и в final-form, мы работаем с данными примерно так:
...
Но поскольку useField имеет имя поля 'name', он может реализовать частичную подписку.
...
Для этого в нашей воображаемой библиотеки должен быть proxy объект. Он будет 
